...
int latchCloseAngle = 180; // Initializes int to store starting servo position
int latchOpenAngle = 90; // Initializes int to store open servo position

int doorCloseAngle = 0; // Initializes int to store starting servo position
int doorOpenAngle = 90; // Initializes int to store open servo position
...
void setup() {
    ...
    latchServo.write(latchCloseAngle);
    doorServo.write(doorCloseAngle);
}

void loop() {
    delay(2000); // Delay, door cant open till latch is open
    latchServo.write(latchOpenAngle); // turns the latch servo 90 degrees

    Serial.println("Latch opened.");

    delay(2000); // Sets delay for robot cleaning cycle
    doorServo.write(doorOpenAngle); // turns the door servo 90 degrees
    ...
    if (digitalRead(sensorPin) == LOW) { // Check if the sensor detects an obstacle
        ...
        doorServo.write(doorCloseAngle); // Close the door if an obstacle is detected
        delay(2000); // Wait for a second before checking again
        latchServo.write(latchCloseAngle); // Close the latch if an obstacle is detected
    }
}
